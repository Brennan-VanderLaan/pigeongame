...
async def get_mesh(mesh_id: UUID, db: AsyncSession = Depends(get_db)):
    ...
    """
    result = await db.execute(
        select(MeshDB).where(MeshDB.id == str(mesh_id)).options(
            selectinload(MeshDB.nodes),
            selectinload(MeshDB.hubs).selectinload(HubDB.spokes)
        )
    )
...
async def delete_mesh(mesh_id: UUID, db: AsyncSession = Depends(get_db)):
    """
    ...
    """
    result = await db.execute(select(MeshDB).where(MeshDB.id == str(mesh_id)))
    mesh = result.scalar_one_or_none()
...
async def remove_node_from_mesh(
    ...
    """
    result = await db.execute(
        select(NodeDB).where(NodeDB.id == node_id, NodeDB.mesh_id == mesh_id)
    )
...
async def create_hub(
    ...
    result = await db.execute(
        select(NodeDB).where(NodeDB.id == str(hub.node_id), NodeDB.mesh_id == str(mesh_id))
    )
    node = result.scalar_one_or_none()
    ...
    result = await db.execute(
        select(HubDB).where(HubDB.node_id == str(hub.node_id), HubDB.mesh_id == str(mesh_id))
    )
...
async def get_hubs(mesh_id: UUID, db: AsyncSession = Depends(get_db)):
    ...
    """
    result = await db.execute(
        select(HubDB).where(HubDB.mesh_id == str(mesh_id)).options(
...
async def remove_hub(
    ...
    """
    result = await db.execute(
        select(HubDB).where(HubDB.id == hub_id, HubDB.mesh_id == mesh_id)
    )
...
async def link_node_to_hub(
    ...
    result = await db.execute(
        select(HubDB).where(HubDB.id == str(link.hub_id), HubDB.mesh_id == str(mesh_id)).options(
            selectinload(HubDB.spokes)
        )
    )
    hub = result.scalar_one_or_none()
    ...
    # Check if node exists in this mesh
    result = await db.execute(
        select(NodeDB).where(NodeDB.id == str(link.node_id), NodeDB.mesh_id == str(mesh_id))
    )
    node = result.scalar_one_or_none()
