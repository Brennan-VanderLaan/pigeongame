...
run_with_status "Nuking existing nodes" docker rm -f talos-node0 talos-node1 talos-node2
...
echo
print_header "SPOOLING CONTAINERS"

run_with_status "Creating controlplane" docker run -d \
  --name talos-node0 \
  --hostname talos-node0 \
  --read-only \
  --privileged \
...
run_with_status "Creating node0" docker run -d \
  --name talos-node1 \
...
run_with_status "Creating node1" docker run -d \
  --name talos-node2 \
  --hostname talos-node2 \
  --read-only \
  --privileged \
  --security-opt seccomp=unconfined \
  --mount type=tmpfs,destination=/run \
